...
class MAE_REIMPL_Wrapper:
    def __init__(self, arch, patch, imsize, extract_mode='none', blk_sel='all'):
        ...
        if imsize != 224:
            ...
            exit(-1)
        self.mod_id = 'MAE-REIMPL-%s-%i-%i' % (arch, patch, imsize)
        if self.mod_id == 'MAE-REIMPL-B-16-224':
            self.checkpoint_file = osp.join(
                path_dir, 'models', 'mae_reimpl', 'mae_reimpl_base16.th')
        elif self.mod_id == 'MAE-REIMPL-L-16-224':
            self.checkpoint_file = osp.join(
                path_dir, 'models', 'mae_reimpl', 'mae_reimpl_large16.th')
        elif self.mod_id == 'MAE-REIMPL-H-16-224':
            self.checkpoint_file = osp.join(
                path_dir, 'models', 'mae_reimpl', 'mae_reimpl_huge16.th')
        else:

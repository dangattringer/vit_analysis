...
class MAE_CT_Wrapper:
    def __init__(self, arch, patch, imsize, extract_mode='none', blk_sel='all'):
        ...
        if imsize != 224:
            ...
            exit(-1)
        self.mod_id = 'MAE-CT-%s-%i-%i' % (arch, patch, imsize)

        if self.mod_id == 'MAE-CT-B-16-224':
            self.checkpoint_file = osp.join(
                path_dir, 'models', 'mae_ct', 'maect_base16.th')
        elif self.mod_id == 'MAE-CT-L-16-224':
            self.checkpoint_file = osp.join(
                path_dir, 'models', 'mae_ct', 'maect_large16.th')
        elif self.mod_id == 'MAE-CT-H-16-224':
            self.checkpoint_file = osp.join(
                path_dir, 'models', 'mae_ct', 'maect_huge16.th')
        else:
